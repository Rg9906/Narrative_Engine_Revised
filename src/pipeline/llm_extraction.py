...
class LLMExtractionEngine:
    ...
    def _build_world_timeline_prompt(self, chapter_num: int, cleaned_text: str) -> str:
        return (
            ...
            f"    {{\n"
            f"      \"event_id\": \"e1\",\n"
            f"      \"summary\": \"One plain-English sentence a reader would recognize as a story "
            f"beat, e.g. 'Sergeant Rhodes arrives at the house with news that a witness has come "
            f"forward.'\",\n"
            f"      \"subject\": \"character_id or item_id\",\n"
            f"      \"predicate\": \"short verb phrase, e.g. 'dies', 'moves_to', 'discovers'\",\n"
            f"      \"object\": \"target of the action\",\n"
            f"      \"participants\": [\"every character_id involved in this beat\"],\n"
            f"      \"location\": \"where it happens, or null\",\n"
            f"      \"time\": \"time or ordering hint mentioned in the text, or null\",\n"
            f"      \"causes\": \"event_id of an earlier event in this same list this one follows "
            f"from, or null\",\n"
            f"      \"event_type\": \"revelation|decision|conflict|arrival|departure|death|"
            f"discovery|promise|betrayal|setback|reunion|other\",\n"
            f"      \"significance\": 0.0,\n"
            f"      \"why_it_matters\": \"One clause on what this changes for the plot or a "
            f"character. If you cannot name what it changes, the event does not belong here.\",\n"
            f"      \"confidence\": 1.0\n"
            f"    }}\n"
            f"  ]\n"
            f"}}\n\n"
            f"CRITICAL — what counts as a timeline event:\n"
            f"This list is a chapter's plot skeleton, read by a human editor. Return AT MOST 8 "
            f"events, and fewer if the chapter genuinely contains fewer. A quiet chapter may "
            f"legitimately have 2.\n"
            f"INCLUDE only beats that change the story's state: something is revealed, decided, "
            f"discovered, promised, broken, lost, or someone arrives, leaves, or dies.\n"
            f"EXCLUDE, always: scene-setting and atmosphere; a character merely perceiving, "
            f"noticing, thinking, remembering or feeling something; ordinary physical business "
            f"(walking across a room, sitting, holding an object, making coffee) unless that act "
            f"is itself the plot beat; and anything you cannot write a 'why_it_matters' clause "
            f"for.\n"
            f"Score `significance` honestly on 0.0-1.0: 0.9+ = the chapter's turning point, "
            f"0.6-0.8 = advances a plotline, 0.4-0.5 = minor but consequential, below 0.4 = it "
            f"should not be in this list at all. Do not inflate scores.\n"
        )
