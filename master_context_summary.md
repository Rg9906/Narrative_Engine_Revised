...
*   **Test Suite**: All **73 unit and integration tests are passing** (grew from 52 as ValidationEngine/ContextRetriever/9-inspector EditorialEngine were added, then to 73 with the coreference-attribution and trait-extraction fixes above — this number will keep moving; check `pytest -q` for the live count rather than trusting this doc).
...
| **Theme/Mystery/Symbol detection** | Refined, still heuristic | Keyword-based with noise-reduction gates; themes/symbols now get an auto-generated `description` field. Not the topic-modeling/ML approach in the original design. |
...
*   **Richer story graph**: `NarrativeGraph.build()` (`src/engines/narrative_graph.py`) currently only emits character↔character relationship edges and event→chapter edges — world elements and themes are exported as nodes but never linked to anything. Wiring real edges (character↔world, character↔theme) would make the new Story Graph page much more connected.
